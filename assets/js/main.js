...
function initializeMap() {
  var map = new google.maps.Map(document.getElementById("map"), {
    zoom: 18,
    center: new google.maps.LatLng(30.0066832, 30.9758812),
    panControl: false,
  ...
  marker = new google.maps.Marker({
    position: new google.maps.LatLng(30.0066832, 30.9758812),
    map: map,
    icon: image,
  });
...
window.onload = function () {
    ...
    if ($(".js-main-slider").length) {
      ...
      $(".js-main-slider").on(
        ...
        function (event, slick, currentSlide, nextSlide) {
          var newCount =
            "<span>" + (nextSlide + 1) + "</span>/" + slick.slideCount;
        ...
        $this
          ...
          .fadeIn($speed, function () {
            ...
            if (
              ...
              $(window).width() -
              $this.closest($navItem).closestDescendent($navHide).offset()
                .left
            ) {
  ...
  $(function () {
    ...
    if ($(".js-selectize-value-1").length) {
      $select1[0].selectize.setValue("Им�? API-ключа");
    }
...
//mo2mena
//if ($(window).width() > 900) {
$(".cart-icon, .sizes li, .cart-close,.cart-overlay,.addtocart").click(
  function () {
...
if ($(window).width() < 900) {
  ...
  //    })
  //    $('.sizes li, .cart-close,.cart-overlay').click(function () {
  //        $('.cart-box').toggleClass('open');
  //    })
}
$(".search-icon,.search-close,.search-form__button,.search-overlay").click(
  function () {
    $(".search-box").toggleClass("open");
  }
);
$(".rate-button,.rate-close,.rate-overlay").click(function () {
  $(".rate-popup").toggleClass("open");
});

$(".fav,.cancel-btn,.confirm-close,.confirm,.confirm-overlay").click(
  function () {
    $(".confirm-popup").toggleClass("open");
  }
);
$(
  ".del.wishlist__remove,.delete-close,.delete-overlay,.alert .close,.rem.button,.delete-popup .button"
).click(function () {
  $(".delete-popup").toggleClass("open");
});

$(".rename,.rename-overlay,.rename-cancel-btn").click(function () {
  $(".rename-popup").toggleClass("open");
});

$(".sizeguide,.sizeguide-close,.sizeguide-overlay").click(function () {
  $(".sizeguide-popup").toggleClass("open");
});
if ($(window).width() < 900) {
  $(".footer-nav__col").click(function () {
    $(this).find(".footer-nav__list").toggleClass("open");
    $(this).toggleClass("min");
  });
  $(".mob-filter,.apply-button").click(function () {
    $(".filters").slideToggle();
  });
}
//eye password
...
$(".colors li").hover(function () {
  $(".pro-color").css("opacity", "0");
  $(".colors li").removeClass("active");
  var target = $(this).index();
  $(this)
    .parents(".short-item__all")
    .find(".pro-color:eq(" + target + ")")
    .css("opacity", "1");
  $(this).addClass("active");
});

// Trigger Megamenu on link hover
$(function () {
  const headerNormalLinks = $(".header .header-nav__link:not([data-megamenuTrigger])");

  const megamenuTriggers = $("[data-megamenuTrigger]"),
    megamenuWrappers = $("[data-megamenu]");

  headerNormalLinks.on("mouseenter", function () {
    megamenuTriggers.removeClass("active");
    if (megamenuWrappers.is(":visible")) {
      megamenuWrappers.hide();
    }
  })
  ...
  if (megamenuTriggers.length !== 0) {
    megamenuTriggers.on("mouseenter", function () {
      ...
      if (megamenuWrappers.is(":visible")) {
        megamenuWrappers.slideUp(200).delay(250);
    ...
    // Hide megamenu on mouse leave it
    megamenuWrappers.on("mouseleave", function () {
      megamenuTriggers.removeClass("active");
      $(this).slideUp();
    });
  }
